...
public class DeviceHomeAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(mDatas.size()>position&&mDatas.get(position)!=null){
            if(mDatas.get(position).getBbu()!=null){
                holder.mTvCellnum.setText(mDatas.get(position).getBbu());
            }
            switch (mDatas.get(position).getBbuStatus()){
                case "0":
            ...
            holder.mTvSync.setText(DataUtils.getSyncState(mDatas.get(position).getSyncstatus()));
        }
        return convertView;
    }
