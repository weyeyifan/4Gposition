...
public class MemLibAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        holder.mCb.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                MemberLibFragment.tempDatas.get(position).setChecked(!MemberLibFragment.tempDatas.get(position).isChecked());
            }
        });
        holder.mTvGender.setText(datas.get(position).getGender()==0?"男":"女");
        holder.mTvPhone.setText(datas.get(position).getPhone());
        holder.mTvState.setText(datas.get(position).getState()==1?"在线":"N/A");
        return convertView;
    }
