...
//                                Pattern pattern = Pattern.compile(reg);
                                if (mTvName.getText().toString().trim().length() == 0) {
                                    ...
                                    boolean tag=true;
                                        for(int i=0;i<MemberLocalFragment.tempDatas.size();i++){
                                            if(MemberLocalFragment.tempDatas.get(i).getImsi().equals(mTvImsi.getText().toString())){
                                                tag=false;
                                            }
                                        }
                                    if(tag){
                                        ThreadUtils.getThreadPoolProxy().execute(new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                memberBean.setRemark(mTvRemark.getText().toString().trim());
                                                MemberLocalFragment.tempDatas.add(memberBean);

                                                add=mDao.add(memberBean);
                                                ThreadUtils.runOnMainThread(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        MemberLocalFragment.adapter.notifyDataSetChanged();
                                                        if(add){
                                                            ToastUtil.show("添加成功!");
                                                        }else{
                                                            ToastUtil.show("添加失败!");
